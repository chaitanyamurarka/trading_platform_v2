...
# Ensure data_module imports are correct if used directly in this file.
# Based on current structure, data is passed into start_optimization_job.
...
def _generate_parameter_combinations(
    ...
    for p_range in parameter_ranges: 
        ...
        try:
            ...
            logger.warning(f"Invalid non-numeric start/end/step for parameter {p_range.name}. Using single value: {p_range.start_value}")
        ...
        if param_type == 'int' or (isinstance(p_start,int) and isinstance(p_end,int) and isinstance(p_step,int) and p_step != 0):
            ...
            if p_step_int > 0:
                current_values = list(range(int(p_start), int(p_end) + 1, p_step_int))
                 ...
                 # Corrected range for negative step to be inclusive
                 current_values = list(range(int(p_start), int(p_end) -1 if p_step_int !=0 else int(p_end), p_step_int)) # if step is 0, this won't run
                 if not current_values and p_start == p_end : current_values = [int(p_start)] # Ensure single value if start=end
                 elif int(p_end) not in current_values and p_start >= p_end : current_values.append(int(p_end)) # ensure end is included for negative steps
            ...
            if p_step == 0: 
             ...
             logger.warning(f"Parameter {p_range.name} has unrecognized type or step. Using single value: {p_range.start_value}")
             current_values = [p_range.start_value]
        
        if not current_values and abs(p_start - p_end) < 1e-9 : # Handle p_start == p_end for floats
            current_values = [round(p_start, 8)]
    ...
    for combo in combinations_dicts:
        ...
        if fast_ema is not None and slow_ema is not None:
            ...
            try:
                ...
                logger.warning(f"Could not compare fast_ema and slow_ema for combo {combo}, adding it anyway.")
                valid_combinations.append(combo) # Or skip if conversion fails
        else: 
            valid_combinations.append(combo)
...
async def _execute_optimization_task(
    ...
    # Progress is 0.0 when starting, will be set to 1.0 on Numba completion.
    # No intermediate progress update from Numba kernel.
    job_status_obj.progress = 0.0 
    logger.info(f"Opt. job {job_id} (Numba) for '{strategy_class.strategy_id}', {len(parameter_combinations)} combos. Status: RUNNING")

    if not historical_data_points:
        job_status_obj.status = "FAILED"; job_status_obj.message = "No historical data provided for optimization."; job_status_obj.end_time = datetime.utcnow(); return
    if not parameter_combinations or (len(parameter_combinations) == 1 and not parameter_combinations[0]): # Check for empty dict
        job_status_obj.status = "FAILED"; job_status_obj.message = "No valid parameter combinations for optimization."; job_status_obj.end_time = datetime.utcnow(); return
    ...
    try:
        ...
        if ohlc_df.empty: raise ValueError("OHLC DataFrame is empty after processing.")
    except Exception as e:
        job_status_obj.status = "FAILED"; job_status_obj.message = f"Data preparation error for optimization: {e}"; job_status_obj.end_time = datetime.utcnow(); return

    n_candles = len(ohlc_df)
    n_combinations = len(parameter_combinations)
    # CORRECTED: Use total_iterations
    job_status_obj.total_iterations = n_combinations 
    ...
    try:
        ...
        # Numba kernel processes all combinations, so set progress to 1.0 on successful completion.
        job_status_obj.progress = 1.0 
        job_status_obj.message = "Numba kernel processing completed successfully." # More positive message

    except Exception as e:
        logger.error(f"Job {job_id}: Error during Numba execution: {e}", exc_info=True)
        job_status_obj.status = "FAILED"; job_status_obj.message = f"Numba kernel execution error: {str(e)}"; job_status_obj.end_time = datetime.utcnow(); return
    ...
    for k in range(n_combinations):
        ...
        job_results_list.append(models.OptimizationResultEntry(parameters=params_for_this_run, performance_metrics=performance_metrics))
...
async def start_optimization_job(
    ...
    for pr_item in request.parameter_ranges:
        ...
        if hasattr(current_pr, 'name') and current_pr.name and "ma" in current_pr.name.lower() and "fast" in current_pr.name.lower():
            if dataset_length > 0: 
                max_allowed_len = dataset_length // 3
                if max_allowed_len > 0 and hasattr(current_pr, 'end_value') and isinstance(current_pr.end_value, (int, float)) and current_pr.end_value > max_allowed_len :
                    logger.info(f"Capping {current_pr.name} max length from {current_pr.end_value} to {max_allowed_len} based on dataset length {dataset_length}")
                    current_pr.end_value = max_allowed_len 
            elif hasattr(current_pr, 'end_value'): 
                 logger.warning(f"Cannot apply dataset length rule to cap MA {current_pr.name} as dataset length is {dataset_length}. Using original end: {current_pr.end_value}")
    ...
    execution_price_type_for_run = request.execution_price_type
    initial_capital_for_run = request.initial_capital
    ...
    background_tasks.add_task(
        ...
        initial_capital_for_run, 
        execution_price_type_for_run 
    )
